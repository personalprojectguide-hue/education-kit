"""
Education Kit -- Flashcards, essay help, study guides, and quiz generators. Learn anything faster.
...
DB_FILE        = "education_kit.db"
SYSTEM_PROMPTS = ['You are an expert Homework Explainer assistant. Enter any concept or homework question. Get a clear step-by-step explanation.', 'You are an expert Personal Statement Writer assistant. Describe your background and goals. Get a compelling university personal statement.', 'You are an expert Flashcard Generator assistant. Paste notes or a topic description. Get a set of Q&A flashcards ready to study.', 'You are an expert Maths Problem Solver assistant. Describe a maths problem. Get a clear solution with every step explained.', 'You are an expert College Application Essay assistant. Describe the prompt and your story. Get a compelling college application essay outline.']

app = FastAPI(title="Education Kit")
...
<div id="auth-overlay">
  <div class="auth-box">
    ...
    <p class="auth-sub">Sign in to use Education Kit</p>
    <div class="auth-tabs">
...
<div id="app">
  <aside class="sidebar">
    <div class="sidebar-hdr">
      ...
      <div class="suite-name">Education Kit</div>
      <div class="suite-tag">Flashcards, essay help, study guides, and quiz generators. Learn anything faster.</div>
    </div>
...
const TOOLS=[{"name": "Homework Explainer", "tagline": "Understand anything, explained simply", "promptTemplate": "Subject: {{subject}}\\nGrade Level: {{grade_level}}\\nQuestion Or Concept: {{question_or_concept}}", "resultFormat": "steps", "inputs": [{"id": "subject", "label": "Subject", "type": "textarea", "placeholder": "Enter subject...", "required": true}, {"id": "grade_level", "label": "Grade Level", "type": "select", "options": ["Elementary", "Middle School", "High School", "College"], "required": false}, {"id": "question_or_concept", "label": "Question Or Concept", "type": "text", "placeholder": "e.g. your question or concept", "required": false}]}, {"name": "Personal Statement Writer", "tagline": "University applications that get offers", "promptTemplate": "Subject: {{subject}}\\nYour Achievements: {{your_achievements}}\\nWhy You'Re Passionate: {{why_you_re_passionate}}\\nCareer Goals: {{career_goals}}", "resultFormat": "text", "inputs": [{"id": "subject", "label": "Subject", "type": "textarea", "placeholder": "Enter subject...", "required": true}, {"id": "your_achievements", "label": "Your Achievements", "type": "textarea", "placeholder": "Enter your achievements...", "required": false}, {"id": "why_you_re_passionate", "label": "Why You'Re Passionate", "type": "text", "placeholder": "e.g. your why you're passionate", "required": false}, {"id": "career_goals", "label": "Career Goals", "type": "select", "options": ["Increase revenue", "Save time", "Reduce costs", "Improve quality", "Learn something new", "Solve a specific problem"], "required": false}]}, {"name": "Flashcard Generator", "tagline": "Turn any text into study flashcards", "promptTemplate": "Source Text Or Topic: {{source_text_or_topic}}\\nNumber Of Cards: {{number_of_cards}}\\nDifficulty Level: {{difficulty_level}}", "resultFormat": "list", "inputs": [{"id": "source_text_or_topic", "label": "Source Text Or Topic", "type": "textarea", "placeholder": "Enter source text or topic...", "required": true}, {"id": "number_of_cards", "label": "Number Of Cards", "type": "select", "options": ["10", "20", "30"], "required": false}, {"id": "difficulty_level", "label": "Difficulty Level", "type": "select", "options": ["Easy", "Medium", "Hard"], "required": false}]}, {"name": "Maths Problem Solver", "tagline": "Step-by-step solutions for any maths problem", "promptTemplate": "Problem: {{problem}}\\nTopic: {{topic}}\\nLevel: {{level}}", "resultFormat": "steps", "inputs": [{"id": "problem", "label": "Problem", "type": "textarea", "placeholder": "Enter problem...", "required": true}, {"id": "topic", "label": "Topic", "type": "text", "placeholder": "e.g. your topic", "required": false}, {"id": "level", "label": "Level", "type": "select", "options": ["Beginner", "Intermediate", "Advanced"], "required": false}]}, {"name": "College Application Essay", "tagline": "Essays that get you into your dream school", "promptTemplate": "Essay Prompt: {{essay_prompt}}\\nYour Story: {{your_story}}\\nValues: {{values}}\\nTarget School: {{target_school}}", "resultFormat": "text", "inputs": [{"id": "essay_prompt", "label": "Essay Prompt", "type": "textarea", "placeholder": "Enter essay prompt...", "required": true}, {"id": "your_story", "label": "Your Story", "type": "text", "placeholder": "e.g. your your story", "required": false}, {"id": "values", "label": "Values", "type": "text", "placeholder": "e.g. your values", "required": false}, {"id": "target_school", "label": "Target School", "type": "text", "placeholder": "e.g. your target school", "required": false}]}];
let currentTool=0;
...
@app.get("/api/health")
def health():
    return {"status": "ok", "app": "Education Kit", "ai_ready": bool(GROQ_API_KEY)}
